...
| p4p (proxy) | `C:\PerforceSandbox\bin\p4p.exe` | same filehost (`.../p4p.exe`) — **pending one-time download**, see `proxy/README.md` |
...
- [~] 9. **Perforce Proxy (p4p)** — **harness complete**: `proxy/start-p4p.ps1`, `stop-p4p.ps1`, `demo-proxy.ps1`, `proxy/README.md` stand up a caching proxy on `:1668` → `:1666` with a measurable cache-fill → cache-hit demo (+ optional `-SeedMB` workload-tier knob). **One gated step remains:** the human-approved `p4p.exe` download (exact command in `proxy/README.md`), then run `demo-proxy.ps1` for live numbers. Closes the roadmap's literal "proxy" gap; see `lessons-learned.md` #13.
