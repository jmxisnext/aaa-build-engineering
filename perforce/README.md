...
P4USER   = devuser
P4CLIENT = devuser-WS01-game-main      # the active stream workspace
```

> **Sandbox identity:** this sandbox standardizes on the neutral P4 user **`devuser`** (studio-portable;
> the scripts default to it). On a fresh box create it as a super user; to migrate an existing `james`
> sandbox, `p4 renameuser --from=james --to=devuser` rewrites the user across changelists, workspaces,
> and protections in one step.
...
`-c devuser-WS01-game-dev` per command, or `$env:P4CLIENT = "..."` per shell.
...
- [x] 4a. Set up a stream depot with mainline / development / release streams — `//game/main`, `//game/dev`, `//game/release-1-0` exist; workspace `devuser-WS01-game-main` synced; first file submitted as change 4
